...
import com.clouyun.charge.modules.system.web.DictController;
 ...
 * 创建日期: 2017年04月19日
 */
public class DictControllerTests extends ApplicationTests{

    @Autowired
    private DictController dictController;//你要测试的Controller
    @Autowired
    ...
    public void getDictLabel(){
        try {
            System.out.println(dictService.getDictLabel("zhzt","01"));
     ...
     */
    @Test
    public void queryLevelDict1() throws Exception {
        //调用接口，传入添加的用户参数
        ResultActions ra = mockMvc.perform(get("/api/dicts/level/1"))
    ...
    public void saveDict() throws Exception {
        String dictJson = FileUtils.read("/modules/system/saveDict.json");
        System.out.println("读取到的Json文件:" + dictJson);
        //调用接口，传入添加的用户参数
        ResultActions ra = mockMvc.perform(post("/api/dicts")
    ...
    public void updateDict() throws Exception {
        String dictJson = FileUtils.read("/modules/system/updateDict.json");
        System.out.println("读取到的Json文件:" + dictJson);
        //调用接口，传入添加的用户参数
        ResultActions ra = mockMvc.perform(put("/api/dicts")
                .contentType(MediaType.APPLICATION_JSON_UTF8)
                .content(dictJson));
        //判断返回值，是否达到预期，测试示例中的返回值的结构如下{"errorCode":0,"errorMsg":"OK"}
        //此处使用jsonPath解析返回值，判断具体的内容
